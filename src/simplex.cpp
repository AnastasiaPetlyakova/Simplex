	...
	for (i = 0; i < num_l; i++) 
		{
			...
			bv[i][0] = i + num_v;
			if (sign[i] == 2) bv[i][1] = fm[i] * (-1); //Если >=, замена знака на противоположный
			else bv[i][1] = fm[i];
		}
	for (i = 0; i < num_v ; i++)
		if (direction) istr[i] = function[i];
		else istr[i] = function[i] * (-1); //Если поимк максимума, то замена знаков на противоположные
}
		
	
void simplex::print_result() //Вывод начальных данных, формирование исходной симплекс-таблиц и поиск опорного решения 
{ 
	int i, j, k;
	double max;
	bool flag=true;
